...
Welcome back! This week's session will introduce you to the most important visualization approaches in R. `r emo::ji("artist_palette")`
...
Later on, we will see how to combine multiple data sources to build a single plot. For now, we will work under the assumption that all of your data live in the same object. Consider using `dplyr::left_join()`, `broom::augment()`, or similar functions to combine data sets or to complement your original data set with information from your models (e.g. fitted values).
...
To do this we will use the function `aes()` within the `ggplot()` function following the data frame.
...
The third layer to render our graph (i.e., to produce a specific type of graph, e.g. bar plot, scatter plot, etc.) is a geometric object. To add one, we need to add a plus (`+`) at the end of the initial line of code and then state the type of geometric object we want to add. For example, `geom_point()` to produce a scatter plot or `geom_bar()` to produce a bar plot. For an overview of the most important functions and geoms available through `ggplot2`, see the `ggplot2` [cheat sheet](https://rstudio.github.io/cheatsheets/data-visualization.pdf).
...
df <- gapminder # let's make a copy of the data 
str(df)
```

## Exercise 1 - Your first plot `r emo::ji("penguin")`

For your first plot using `ggplot2`, we will use the `penguins` data set.
...
1.  Data
2.  Aesthetics
3.  A geometric object (`geom_point()`)

Once we have our scatter plot, adapt the code to:

1.  Use color to convey another variable (e.g., the species of penguin)
2.  Change the axes names
3.  Render the graph with `theme_minimal()`


```{r, figures-side, fig.show="hold",  warning=F}
...
That was the first step towards understanding the basic structure of the layers. Let's have a closer look at what plot types makes sense in which situations. The question is, how can we convey the information most effectively?
...
-   `geom_bar()`: renders bar plots and in plotting distributions behaves in a very similar way from `geom_histogram()` (can also be used with two dimensions)
...
Histograms show the distribution of continuous variables. In this first example, we graph the distribution of the life expectancy variable (i.e., `lifeExp`) from the gapminder dataset.
...
The default number of bins is 30, which means that the entire range of the variable (here 23.60 to 82.60) is split into 30 equally spaced bins. We can change the number of bins manually. Below, we specify 60 bins to approximate a bin width of 1 year, **taking into account the range of the variable** `lifeExp`.

```{r,  warning = F, message = F, fig.height = 3}
max(df$lifeExp) - min(df$lifeExp) # the range is approx 60 years

ggplot(df, aes(x = lifeExp)) +
  geom_histogram(bins = 60)
```

What would happen if we specified 5 bins? Try it out. 
...
-   The position of the whiskers (vertical lines) denote the first quantile value minus 1.5 times the interquartile range and the third quantile value plus 1.5 times the interquartile range.
...
A violin plot is a compact display of a continuous distribution. It is a blend of `geom_boxplot()` and `geom_density()`: a violin plot is a mirrored density plot displayed in a similar way as a boxplot.
...
Create a histogram showing the distribution of penguin body mass using the `penguins` dataset. As you explore the plot, try the following:

1. Choose an appropriate number of bins (for example, start with bins = 15 and adjust).

1. Set a fill color using fill = "red" or a hex code such as "#FF6666".

1. Experiment with different geoms by replacing geom_histogram() with options like geom_density(), geom_bar(), or others.
...
1. Now, look for a variable in the dataset that can be used to meaningfully group body mass. Use that variable to create a box plot or violin plot of body mass.

```{r}
# YOUR CODE HERE
ggplot(penguins, aes(x = species, y = flipper_length_mm)) +
  geom_boxplot() +
  geom_jitter(alpha = 0.6, aes(col = species)) +
  coord_flip() +
  theme_minimal()
```
...
The plot above shows a large amount of clustering on the left side of the plot, while the right side of the plot is sparsely populated. This makes it hard to gauge the relationship between the two variables. Below, we make a few adjustments to the graph to better display the relationship.
...
One reason why the plot above is hard to read is rooted in the shape of the distribution of the GDP per capita variable. GDP per capita has a strong right skew (yes, right, look at where the *tail* of the distribution is). Below we are plotting the average on top of the graph using the `geom_vline()`.
...
ggplot(df,
  ...
  geom_vline(xintercept = av, color = "red") #try just typing 7215
```
...
gridExtra::grid.arrange(log10_direct_plot, log10_via_scale_plot, ncol = 2)
```
...
We can use the same principle in bivariate (or multivariate) displays of data. Below, we use the `scale` transformation on the variable and reflect it in the axis label clarify that it is the relationship between life expectancy and the logarithm of GDP per capita that has a strong positive relationship.
...
The default smoothing method is `loess` for less than 1,000 observations and `gam` (Generalized Additive Models) for observations greater or equal to 1,000. `ggplot2` informs us which smoothing method was used via a message. By default, a 95% confidence interval is added to the trend line.
...
To compare a time series across many units, we can plot all series in one figure using thin, low-opacity lines to keep the display readable. Here, we apply this approach to life expectancy over time and highlight China in red.
...
ggplot(df,
  ...
  labs(title = "Life expectancy over time",
       ...
       y = "Life expectancy") +
  theme_light() +
  geom_line(data = subset(df, country %in% "China"),
...
cors <- df |>  
  ...
  dplyr::summarize(cor = cor(lifeExp, log10(gdpPercap)))
```
...
globalpop <- df |> 
  ...
  dplyr::summarize(pop_tot = sum(as.numeric(pop)))
...
`ggplot2()` is pretty nice and it just stacked each continent's population on top of each other. If we instead wanted a separate bar for each continent, we can use the `position` parameter within `geom_col()`.
...
Suppose we wanted to know which countries in Europe are shrinking and which countries are growing in terms of their population. We can use our data wrangling skills to compute the most recent change in population by taking the current value and subtracting the previous year's value.

```{r}
diff07 <- df |> 
  dplyr::group_by(country) |>
  dplyr::arrange(year) |> 
  dplyr::mutate(fd = pop - dplyr::lag(pop))
```

Below, we plot the change in population for European countries in 2007.
...
This is really hard to see. Let's flip the axes using `coord_flip()`. This could useful because countries are ordered alphabetically, but visually, it is confusing. Let's reorder the country axis based on the value of the population change. The default is to order the points in ascending order from the origin.

```{r, fig.show="hold",  fig.height = 6 , warning=F, message=F}
ggplot(subset(diff07, continent == "Europe" & year == 2007),
       aes(x = country, y = fd)) +
  geom_col() +
  coord_flip() +
...
Create a scatter plot to explore the relationship between `flipper_length_mm` and `body_mass_g`. Then, color by a grouping variable (of your choice) and add linear trend lines.
...
ggplot(penguins, aes(x = flipper_length_mm, y = body_mass_g, col = species)) +
  ...
  theme_light() +
  geom_smooth(method = "lm", linewidth = 0.5, se = FALSE, alpha = 0.2)
```
...
We can also use hexadecimal or RGB (red, green, blue) strings to specify colors. There are plenty of online tools to pick colors and extract hexadecimal or RBG strings. One of our favorites is [this one](http://www.colorhexa.com). This online tool allows you to specify a color name, hexadecimal, or RGB string, and returns information on color schemes, complementary colors, as well as alternative shades, tints, and tones. It also offers a color blindness simulator.
...
There are a ton of resources and packages with pre-defined color schemes. The most popular is [colorbrewer2](https://colorbrewer2.org/#type=sequential&scheme=BuGn&n=3). You can either pick the desired colors manually, or use the `scale_color_brewer()` function in `ggplot2()`.
...
Sometimes you will be printing on a gray scale. This means that color will not be enough to differentiate five lines. We can use different line types instead by specifying the `linetype` parameter within the `aes()` argument. This also makes the graph more color blind friendly. Notice below that in order to combine the legends for the `linetype` and `color` aesthetics, we need to pass the same name within the `scale` function.
...
Create a plot to compare the GDP per capita development of the BRICS countries (Brazil, Russia, India, China, South Africa). Unfortunately, Russia (or previously the Soviet Union) is not part of the `gapminder` data, so we cannot display it in the plot.
...
Now that you have been introduced to some of the basics of `ggplot2`, **the best way to move forward is to experiment**. As we have discussed before, the **R** community is very open. Perhaps, you can gather some inspiration from the [Tidy Tuesday](https://github.com/rfordatascience/tidytuesday) social data project in R where users explore a new dataset each week and share their visualizations and code on Twitter under #TidyTuesday. You can explore some of the previous visualizations [here](https://shiny.rstudio.com/gallery/tidy-tuesday.html) and try to replicate their code.

[Here](https://github.com/erikgahner/awesome-ggplot2) and [here](https://exts.ggplot2.tidyverse.org/gallery/) are curated lists of awesome `ggplot2` resources. Other cool plot forms to check out are, for example, parallel plots, spaghetti plots, interactive plots, maps, three dimensional plots, network graphs, etc. Of course, there will also be some really cool visualization content in the workshops!!
...
This tutorial is based largely on chapters 7 to 10 from the [Quantitative Politics with R book](https://github.com/erikgahner/qpolr/blob/master/README.md) and [Wilkinson, L., 2012. The grammar of graphics](https://link.springer.com/book/10.1007/0-387-28695-0). In Handbook of Computational Statistics (pp. 375-414). Springer, Berlin, Heidelberg.
