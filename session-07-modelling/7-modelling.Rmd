...
pacman::p_load(kableExtra, # for nicely outputted HTML tables
               ...
               wesanderson,
               tidymodel) # color palette for ggplot2
...
formula_string <- paste("life_expectancy", "~ gdp + population + alcohol")
formula <- as.formula(formula_string)
reg_1 <- lm(formula, data = life_expec_dat) 
tidy(reg_1)

formula_string2 <- paste("life_expectancy", "~ gdp * population + alcohol")
formula2 <- as.formula(formula_string2)
reg_2 <- lm(formula2, data = life_expec_dat) 
tidy(reg_2)
...
```{r, echo= FALSE}
mod_table1 <- modelsummary::modelsummary(multiv_model_out[29:31], 
                                        output = "kableExtra",
                                        fmt = fmt_significant(2),  # 2-digits and trailing 0  
                                        estimate  = "{estimate}",
                                        statistic = "conf.int",
                                        coef_omit = "Intercept",
                                        coef_rename=c("gdp"="Gdp", 
